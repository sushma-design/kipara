...
$invoiceno =0;
$select = mysql_query("SELECT estimationno FROM online_estimation ORDER BY estimationno DESC");
$row = mysql_fetch_array($select);
$estimationno1 = $row['estimationno'];
$estimationno2 = $estimationno1 + 1; 

$estimationno = $estimationno2."/".$month."/".$year."-".$year1;
...
// echo "INSERT INTO `online_estimation`(`estimationno`, `slno`, `contactcode`, `date`,`time`, `orderno`,`orderdate`, `dispatch`, `sreff`,`oreff`, `destination`,`transportername`,`couriername`, `note`,`terms`,`category`, `item`,`itemname`,`unit`, `hsn`,`prevstock`,`qty`,`rate`,`total`,`grandtotal`,`transport`,`packing`,`labour`,`totalamt`,`discount`,`finaltotal`,`paymentmode`,`chequeno`,
// `chequedate`,`bankname`,`batchnumber`,`cardtype`,`drum`,`paidamt`,`pendingamt`,`user`) VALUES
// ('$estimationno','$estimationno','$slno','$contactcode','$date','$time','$orderno','$orderdate','$dispatch','','$oreff','$destination','$transportername','$couriername','$challan','$terms','$category','$itemid','$item','$unitnew','$hsn','$stock','$qty','$rate','$total','$grandtotal','$transport','$packing','$labour','$totalamt','$discount','$finaltotal','$paymentmode','$cheque','$chequedate','$bankname','$batchnumber','$cardtype','$drum','$givenamt','$pendingamt','$user')";
    $insert = "INSERT INTO `online_estimation`(`estimationno`, `slno`, `contactcode`, `date`,`time`, `orderno`,`orderdate`, `dispatch`, `sreff`,`oreff`, `destination`,`transportername`,`couriername`, `note`,`terms`,`category`, `item`,`itemname`,`unit`, `hsn`,`prevstock`,`qty`,`rate`,`total`,`grandtotal`,`transport`,`packing`,`labour`,`totalamt`,`discount`,`finaltotal`,`paymentmode`,`chequeno`,
	`chequedate`,`bankname`,`batchnumber`,`cardtype`,`drum`,`paidamt`,`pendingamt`,`user`) VALUES
	('$estimationno','$estimationno','$slno','$contactcode','$date','$time','$orderno','$orderdate','$dispatch','','$oreff','$destination','$transportername','$couriername','$challan','$terms','$category','$itemid','$item','$unitnew','$hsn','$stock','$qty','$rate','$total','$grandtotal','$transport','$packing','$labour','$totalamt','$discount','$finaltotal','$paymentmode','$cheque','$chequedate','$bankname','$batchnumber','$cardtype','$drum','$givenamt','$pendingamt','$user')";
...
<div class="col-xs-12 nopadding">
    <table class="printTable" width="100%"
        ...
        <tbody>
        <tr height="25">
            <td style="border-bottom: 1px solid" colspan="4" align="center">ESTIMATION </td>
        </tr>
        <tr>
            ...
            <td width="30%" style="border-top: 1px solid #000000;border-right: 1px solid #000000;border-left: 1px solid #000000;" align="left">
                ...
                <label style="font-size: 14px;padding-left: 5px;">ESTIMATION NO : <?php echo $estimationno; ?></label> <br/>
                <label style="font-size: 14px;padding-left: 5px;">Delivery Challan : <?php echo $counter?> </label> <br/>
    ...
    <table class="printTable" width="100%" height="120">
        ...
        <tr>
            ...
            <td style="border-bottom: hidden;" width="30%">
                <label style="font-size: 14px;padding-left: 5px;" colspan="2">Dispatch Through: <?php echo ucfirst($dispatch);?></label> </td>
    ...
    <table border='1' width='100%' height="300" align='center' bordercolor='#000000'
        ...
        <tr>
         ...
         <!-- <tr height="25px;">
            ...
            <td align="right" style="padding-right: 10px;"><label style="padding-top: 3px;"><?php echo round($total); ?></label></td>
        </tr> -->
        <!-- <tr height="25px;">
            <td colspan="7" style="border-right: 1px solid #000000;text-align: right; padding-right:10px;"><strong>Discount</strong></td>

            <td align="right" style="padding-right: 10px;"><label style="padding-top: 3px;"><?php echo round($discount); ?></label></td>
        </tr> -->
        <tr height="25px;">
            <td colspan="7" style="border-right: 1px solid #000000;text-align: right; padding-right:10px;"><strong>Total</strong></td>

            <td align="right" style="padding-right: 10px;"><strong><input readonly type="text" name="Net_Total" id="Net_Total" value="<?php echo round($total);?>" style="border:hidden; text-align: right;"></strong></td>
        </tr>
    ...
    <table border='1' align='center' width="100%" bordercolor='#000000'
        ...
        <tr style="border-bottom: hidden">
            <td style="padding-left: 5px;"><p style="font-size: 12px;">
                    1. We declare that this estimation shows the actual price of goods described and has
